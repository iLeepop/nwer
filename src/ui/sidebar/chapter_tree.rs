...
use crate::storage::{ChapterNodeKind, ChapterTreeNode, MoveDirection};
...
pub fn render_chapter_tree(state: &AppState, cx: &mut Context<'_, Workspace>) -> impl IntoElement {
    ...
    v_flex()
        ...
        .child(
            v_flex()
                ...
                } else if state.chapter_tree.is_empty() {
                    vec![
                        div()
                            .p_2()
                            .text_sm()
...
fn render_node(
    ...
    let label = match kind {
        ChapterNodeKind::Directory => node.name.clone(),
        ChapterNodeKind::Chapter => node.title.clone().unwrap_or_else(|| node.name.clone()),
    };
    let expanded = state.is_expanded(&rel);
    let selected = state.ui.selected_node.as_deref() == Some(rel.as_str());
    let is_dir = kind == ChapterNodeKind::Directory;
