...
use super::chapter_store::{load_chapter, save_chapter};
...
/// 相对 `chapters/` 的路径，使用 `/` 分隔（空字符串表示根）。
pub type RelPath = String;

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ChapterNodeKind {
    Directory,
    Chapter,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ChapterTreeNode {
    /// 相对 `chapters/` 的路径（章节含 `.json`）。
    pub rel_path: RelPath,
    /// 目录名为文件夹名；章节为去掉 `.json` 的文件名。
    pub name: String,
    pub kind: ChapterNodeKind,
    pub chapter_id: Option<Uuid>,
    pub title: Option<String>,
    pub children: Vec<ChapterTreeNode>,
}

impl ChapterTreeNode {
    pub fn is_directory(&self) -> bool {
        self.kind == ChapterNodeKind::Directory
    }

    pub fn is_chapter(&self) -> bool {
        self.kind == ChapterNodeKind::Chapter
    }
}
...
pub fn resolve_rel(project_dir: &Path, rel: &str) -> Result<PathBuf> {
    ...
    if !path.starts_with(&root) {
        bail!("path escapes chapters root: {rel}");
    }
    Ok(path)
}

pub fn scan_chapter_tree(project_dir: &Path) -> Result<Vec<ChapterTreeNode>> {
    let root = chapters_dir(project_dir);
    if !root.exists() {
        return Ok(Vec::new());
    }
...
fn scan_dir(abs: &Path, rel: &str) -> Result<Vec<ChapterTreeNode>> {
    ...
    for entry in entries {
        ...
        if ft.is_dir() {
            ...
            nodes.push(ChapterTreeNode {
                rel_path: child_rel,
                name: name.into_owned(),
                kind: ChapterNodeKind::Directory,
                chapter_id: None,
                title: None,
                children,
            });
        } else if ft.is_file() && name.ends_with(".json") {
            let chapter = load_chapter(&entry.path())?;
            let stem = name.trim_end_matches(".json").to_string();
            nodes.push(ChapterTreeNode {
                rel_path: child_rel,
                name: stem,
                kind: ChapterNodeKind::Chapter,
...
pub fn create_directory(
    ...
    let chapters_root = chapters_dir(project_dir);
...
/// 在 `parent_rel` 下新建章节文件 `{name}.json`。
pub fn create_chapter_file(
    project_dir: &Path,
    parent_rel: &str,
    name: &str,
    chapter: &Chapter,
    ...
    if dest.exists() {
        bail!("chapter already exists: {}", dest.display());
    }
    if !parent.exists() {
        fs::create_dir_all(&parent)
            .with_context(|| format!("failed to create parent {}", parent.display()))?;
    }
    save_chapter(&dest, chapter)?;
...
pub fn copy_chapter(
    ...
    max_depth: u32,
    now: DateTime<Utc>,
) -> Result<(RelPath, Chapter)> {
    let src = resolve_rel(project_dir, src_rel)?;
    if !src.is_file() {
        bail!("copy source must be a chapter file: {src_rel}");
    }
    let mut chapter = load_chapter(&src)?;
    chapter.id = Uuid::now_v7();
    // touch block timestamps is out of scope; keep content
    let _ = now;
    let rel = create_chapter_file(project_dir, dest_parent_rel, new_name, &chapter, max_depth)?;
    Ok((rel, chapter))
}
...
pub fn find_node_by_chapter_id(nodes: &[ChapterTreeNode], id: Uuid) -> Option<&ChapterTreeNode> {
    for n in nodes {
        if n.chapter_id == Some(id) {
            return Some(n);
        }
        if let Some(found) = find_node_by_chapter_id(&n.children, id) {
            return Some(found);
        }
    }
    None
...
pub fn find_node_by_rel<'a>(
    nodes: &'a [ChapterTreeNode],
    rel: &str,
) -> Option<&'a ChapterTreeNode> {
    for n in nodes {
        if n.rel_path == rel {
            return Some(n);
        }
...
mod tests {
    ...
    #[test]
    fn copy_chapter_creates_new_id() {
        let (_t, project_dir) = setup();
        let max = 3u32;
        let ch = Chapter::new("原稿", now());
        let src_id = ch.id;
        create_chapter_file(&project_dir, "", "ch-001原稿", &ch, max).unwrap();

        let (new_rel, copied) = copy_chapter(
            &project_dir,
            "ch-001原稿.json",
            "",
            "ch-002副本",
    ...
    fn move_chapter_across_dirs_and_reject_mixed() {
        ...
        assert!(err.is_err(), "should reject chapter next to directories");
    }
    ...
    fn move_rejects_depth_exceeded_for_subtree() {
        let (_t, project_dir) = setup();
        let max = 3u32;
        // chapters/a/b/ch.json  height of a = 3
        create_directory(&project_dir, "", "a", max).unwrap();
        create_directory(&project_dir, "a", "b", max).unwrap();
        create_chapter_file(
            &project_dir,
    ...
    fn move_sibling_up_down_swaps_order() {
        let (_t, project_dir) = setup();
        let max = 3u32;
        create_chapter_file(&project_dir, "", "ch-001", &Chapter::new("一", now()), max).unwrap();
        create_chapter_file(&project_dir, "", "ch-002", &Chapter::new("二", now()), max).unwrap();
    ...
    fn rename_chapter_file() {
        let (_t, project_dir) = setup();
        create_chapter_file(&project_dir, "", "ch-001旧", &Chapter::new("旧", now()), 3).unwrap();
        let new_rel = rename_node(&project_dir, "ch-001旧.json", "ch-002新").unwrap();
        assert_eq!(new_rel, "ch-002新.json");
        let ch = load_chapter(&resolve_rel(&project_dir, &new_rel).unwrap()).unwrap();
