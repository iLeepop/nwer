...
pub fn outline_entry_path(project_dir: &Path, category: OutlineCategory, key: &str) -> PathBuf {
    project_dir
        .join("outline")
        .join(category.label())
...
mod tests {
    ...
    fn outline_entry_path_uses_category_label() {
        let path = outline_entry_path(Path::new("/novel"), OutlineCategory::Character, "张三");
    ...
    fn outline_save_load_roundtrip() {
        ...
        let path = dir.path().join("角色").join("张三.json");
